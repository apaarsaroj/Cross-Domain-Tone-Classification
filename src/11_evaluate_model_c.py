...
label_to_idx_c = mapping_c_full['label_to_idx']
idx_to_label_c = {int(k): v for k, v in mapping_c_full['idx_to_label'].items()}

num_labels_c = len(label_to_idx_c)
print(f"Model C loaded: {num_labels_c} labels")
print(f"Label mapping: {len(label_to_idx_c)} original labels")
...
df_val_c = pd.read_csv(val_c_file)
print(f"Validation set: {len(df_val_c)} samples")
print(f"Original label distribution:")
print(df_val_c['label'].value_counts().head(10))
...
with torch.no_grad():
    for i in tqdm(range(0, len(df_val_c), batch_size), desc="Inference"):
        batch_texts = df_val_c['text'].iloc[i:i+batch_size].tolist()
...
predictions_c_labels = [idx_to_label_c[pred] for pred in predictions_c_raw]
true_labels = df_val_c['label'].values

# Calculate accuracy (exact match on original labels)
correct = sum(p == t for p, t in zip(predictions_c_labels, true_labels))
...
for i in range(min(10, len(df_val_c))):
    print(f"  True: {true_labels[i]:<20} Pred: {predictions_c_labels[i]:<20} {'✓' if true_labels[i] == predictions_c_labels[i] else '✗'}")
...
predictions_test = [optimal_mapping.get(int(pred), 2) for pred in predictions_test_raw]  # Default to Casual
...
mapping_file = os.path.join(results_dir, 'optimal_mapping_c_v2.json')
mapping_named = {idx_to_label_c.get(int(k), f"label_{k}"): int(v) for k, v in optimal_mapping.items()}
...
df_results = pd.DataFrame({
    ...
    'pred_c': predictions_test
})
results_file = os.path.join(results_dir, 'test_results_model_c_v2.csv')
